...
namespace WorkerRole1
{
    ...
    {
        public FileMan() { }
        private static CloudStorageAccount CloudStorageAccountstorageAccount;
        ...
        public void  makeFiles(string SPC,string structureContent,string behaviourContent)
        {
            try
            {
                CloudStorageAccountstorageAccount = CloudStorageAccount.Parse("DefaultEndpointsProtocol=https;AccountName=livecrawlsto;AccountKey=svaV/KdPuPXI6Qcqw0rjtVqQmu49klAKRr+N3/nBlGPjInRd1vMSlV1kk4mUBEO6WpqYf5g1mE88DgNWOILogg==;EndpointSuffix=core.windows.net");
                CloudFileClientfileClient = CloudStorageAccountstorageAccount.CreateCloudFileClient();
                CloudFileSharefileShare = CloudFileClientfileClient.GetShareReference("rfl");
                if (CloudFileSharefileShare.Exists())
                {
                    ...
                    Trace.TraceInformation("1-"+ rootDirectory.Name);
                    Trace.TraceInformation("2-"+ rootDirectory.Properties);
                    Trace.TraceInformation("3-"+ rootDirectory.Uri);
                    Trace.TraceInformation("4-"+ rootDirectory.ToString());
                    Trace.TraceInformation("5-"+ rootDirectory.StorageUri);
